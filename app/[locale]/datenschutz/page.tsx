...
export async function generateMetadata({
  ...
  return {
    title: t("title"),
    description: "Privacy policy for HookTap.",
    robots: {
      index: false,
      follow: false,
    },
...
export default async function DatenschutzPage({
  ...
  const sections = t.raw("privacy.sections") as { title: string; text: string }[];
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-14 text-white md:px-8">
      ...
      <h1 className="mt-4 text-4xl font-bold md:text-5xl">{t("privacy.title")}</h1>
      <p className="mt-4 text-white/65">{t("privacy.subtitle")}</p>
