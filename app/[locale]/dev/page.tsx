    ...
    <main className="relative overflow-x-clip min-h-screen">
      ...
      <div className="mx-auto max-w-4xl px-6 py-8 md:px-8 lg:px-10 space-y-16 md:space-y-20">
        ...
        <motion.section initial="hidden" whileInView="show" viewport={vp} variants={fadeUp} transition={{ duration: 0.5 }}>
          ...
          {/* Code block */}
          <div className="rounded-[1.5rem] overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.09)" }}>
            {/* Tab bar */}
            <div className="flex items-center gap-1 px-4 pt-3 pb-0" style={{ background: "rgba(10,10,12,0.95)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
              {tabs.map(({ key, label }) => (
                <button
                  key={key}
                  onClick={() => setActiveTab(key)}
                  className="px-4 py-2.5 text-xs font-medium rounded-t-lg transition-all"
                  style={{
              ...
              <button
                onClick={copyCode}
                className="mb-1 flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[11px] font-medium transition-all"
                style={{
